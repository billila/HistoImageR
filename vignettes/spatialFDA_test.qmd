---
title: "Reproduce spatialFDA vignette with H&E slide features"
author: "Sehyun Oh"
...
```{r get_all_json_to_spe, echo=FALSE}
...
pathToFiles <- "~/Projects/imageTCGAAnalyses/data/demo_outputs"
```



```{r}
patiendIDs <- stringr::str_extract(fnames, "TCGA-\\d{2}-\\d{4}")
spelist <- vector(mode = "list", length = length(fnames))
names(spelist) <- patiendIDs

for (i in seq_along(fnames)) {
    # pathToFiles <- "https://u24-cancer-genomics.seandavi.workers.dev/hovernet/TCGA_OV/json"
    json_data <- fromJSON(file.path(pathToFiles, paste0(fnames[i], ".json")))
    cells <- json_data[[2]]
    randomSubset <- sample(seq_along(cells), round(length(cells)/10, 0)) # Use only 1/10 of cells for demo
